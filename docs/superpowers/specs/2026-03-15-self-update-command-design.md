...
**`vibepit.dev/releases/0.2.0.json`:**
...
- **Version string convention:** Version strings are bare (without `v` prefix)
  everywhere: JSON payloads, metadata file paths (e.g., `0.2.0.json`), and
  changelog files. Git tags use the `v` prefix (e.g., `v0.2.0`). Since
  `golang.org/x/mod/semver` requires a `v` prefix for all operations, the
  implementation prepends `v` to bare version strings before comparison.
...
1. Read the changelog from `docs/changelogs/{VERSION}.yml`.
...
4. Render the changelog YAML into a plain text string for the version JSON.
   ...
   `Fixed`, etc.), each entry as `- {msg}`. PR and issue references are rendered as
   GitHub links after the message.
5. Write `docs/content/releases/{VERSION}.json` and update the appropriate
...
Structured YAML files at `docs/changelogs/{VERSION}.yml` using keep-a-changelog
categories. Each entry is a map to support structured metadata.

**`docs/changelogs/0.2.0.yml`:**

```yaml
version: "0.2.0"
added:
  - msg: Self-update command for binary and images
    pr: 42
  - msg: Cosign signature verification for downloads
    pr: 45
changed:
  - msg: Combined binary and image update into single command
    pr: 43
fixed:
  - msg: Proxy DNS resolution on macOS
    issue: 38
    pr: 40
```
...
1. If `--use` is set, fetch `releases/{VERSION}.json` directly and skip
