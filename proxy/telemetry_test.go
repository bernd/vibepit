...
func TestDeriveEventMetrics_CodexAPIRequest(t *testing.T) {
	...
	assert.Equal(t, 1.0, byName["api.count{model=o3}"])
	assert.Equal(t, 2000.0, byName["api.duration{model=o3}"])
	assert.Equal(t, 1.0, byName["event_type.count{type=codex.api_request}"])
	assert.Equal(t, 2000.0, byName["event_type.duration{type=codex.api_request}"])
}
...
func TestDeriveEventMetrics_CodexToolResult(t *testing.T) {
	...
	assert.Equal(t, 1.0, byName["tool.count{type=shell}"])
	assert.Equal(t, 150.0, byName["tool.duration{type=shell}"])
}
